...
functionArray getFunctions(stringArray *strArray) {
  ...
  size_t length = 0, resulted_array_counter = 0, k = 1;
  for (size_t i = 0; i < strArray->capacity; i++) {
    if (i + 1 == strArray->capacity) {
      k = 0;
    }
    char *nameFunction;
    length = strlen(strArray->string[i].text);
    if (strArray->string[i].text[length - 1] == '{' ||
        strArray->string[i + k].text[0] ==
            '{') { // Если в конце строки стоит { или в начале следующей строки
                   // стоит {
      if ((nameFunction = isFunction(*strArray, (int)i, (int)length)) !=
          NULL) { // Если в строке есть функция - вернет название функции, иначе
                  // NULL
        if (nameFunction[0] ==
